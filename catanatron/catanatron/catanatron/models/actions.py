...
def robber_possibilities(state, color) -> List[Action]:
    friendly_robber = getattr(state, "friendly_robber", False)
    ...
    for coordinate, tile in state.board.map.land_tiles.items():
        ...
        # Friendly robber: skip tiles where ALL buildings belong to
        # players with < 3 visible VP (can't place robber there).
        if friendly_robber:
            building_colors = set()
            for node_id in tile.nodes.values():
                building = state.board.buildings.get(node_id, None)
                if building is not None and building[0] != color:
                    building_colors.add(building[0])
            if building_colors and all(
                get_visible_victory_points(state, c) < 3 for c in building_colors
            ):
                continue
