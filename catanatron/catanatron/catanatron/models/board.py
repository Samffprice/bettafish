...
class Board:
    ...
    def copy(self):
        ...
        new_cc = defaultdict(list)
        for color, components in self.connected_components.items():
            new_cc[color] = [s.copy() for s in components]
        board.connected_components = new_cc
        ...
        board.buildable_edges_cache = {
            c: edges.copy() for c, edges in self.buildable_edges_cache.items()
        }
        board.player_port_resources_cache = {
            c: s.copy() for c, s in self.player_port_resources_cache.items()
        }
        return board
