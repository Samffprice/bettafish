...
def execute_spectrum(game: Game, action: Action, max_dice_outcomes=None):
    """Returns [(game_copy, proba), ...] tuples for result of given action.
    Result probas should add up to 1. Does not modify self.

    Args:
        max_dice_outcomes: If set, only keep the N most probable dice outcomes
            and renormalize probabilities. Reduces ROLL branching from 11 to N.
    """
    if action.action_type in DETERMINISTIC_ACTIONS:
        ...
        if max_dice_outcomes is not None and len(results) > max_dice_outcomes:
            ...
            results = [(g, p / total_p) for g, p in results]
...
def expand_spectrum(game, actions, max_dice_outcomes=None):
    """Consumes game if playable_actions not specified"""
    children = defaultdict(list)
    for action in actions:
        outprobas = execute_spectrum(game, action, max_dice_outcomes)
